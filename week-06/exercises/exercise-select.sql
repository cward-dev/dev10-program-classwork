...
-- select last_name, email_address, customer_since from customer where YEAR(customer_since) = 2019;
...
-- Expected: 100 Rows, 3 columns
-- select last_name, email_address, customer_since from customer where year(customer_since) = 2019 order by customer_since desc;
...
-- Expected: Woodstock
-- select distinct city from customer order by city desc limit 1;
...
-- Expected: 874 Rows
-- select last_name, first_name, postal_code from customer where postal_code not in ('M3H', 'K7R', 'L2V', 'K7S', 'J6A');

-- Find customer whose last name starts with 'M'.
-- Expected: 76 Rows
-- select last_name, first_name from customer where last_name like 'M%';

-- Find customers with a `(952)` phone area code.
-- Expected: 5 Rows.
-- select last_name, first_name, phone from customer where phone like '(952)%';

-- Find customers with a '.com' email_address
-- Expected: 599 Rows.
-- select last_name, first_name, email_address from customer where email_address like '%.com';

-- Which customers don't have a phone number?
-- Expected: 68 Rows.
-- select last_name, first_name, phone from customer where phone is null or phone = '';

-- Which employees don't have an end_date?
-- In other words, end_date has the null value.
-- Expected: 31 Rows.
-- select last_name, first_name, end_date from employee where end_date is null;

-- Find all projects with `Gabion` in the description.
-- Expected: 179 Rows.
-- select project_id, description from project where description like '%Gabion%';
...
-- select * from project_item order by quantity desc;
...
-- Expected: 1 Row
-- select customer_id, last_name, first_name from customer where last_name = 'Ellett' and first_name = 'Mina';

-- Step 2: use the customer_id to find a record in the login table.
-- Expected: 0 Rows (no login)
-- select customer_id, user_name, password_hash from login where customer_id = 518;

-- Which employees work on projects for customer Tadeo Divine?
-- Step 1: find Tadeo Divine's customer_id
-- Expected: 1 Row
select customer_id from customer where last_name = 'Divine' and first_name = 'Tadeo';

-- Step 2: use their customer_id to find their records in the project table.
-- Expected: 1 Row
select project_id, customer_id from project where customer_id = '854';

-- Step 3: use the project_id from project to find records in project_employee
-- Expected: 4 Rows
select project_id, employee_id from project_employee where project_id = 182;

-- Step 4: use the employee_ids from project_employee to find records in employee
-- Expected: 4 Rows
select employee_id, last_name, first_name from employee where employee_id in (9, 15, 22, 23);