...
class PetServiceTest {
    ...
    void shouldUpdate() {
        // Arrange
        Pet petIn = new Pet(1, "Ula", "Mantis");

        // 4. Stub a specific behavior.
        when(repository.update(petIn)).thenReturn(true);

        // Act
        Result<Pet> result = service.update(petIn);
    ...
    void shouldNotUpdateNotExisting() {
        // Arrange
        Pet petIn = new Pet(10, "Tester", "Testeroni");

        // 4. Stub a specific behavior.
        when(repository.update(petIn)).thenReturn(false);
